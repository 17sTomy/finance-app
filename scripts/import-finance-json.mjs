...
const payload = {
  ...
  fixed_expenses: (database.fixedExpenses ?? []).map((item) => ({ id: fixedIds.get(item.id), name: item.name, amount: item.amount, currency: item.currency, category_id: categoryIds.get(item.categoryId) ?? null, start_date: item.startDate, due_day: item.dueDay, duration_type: item.duration.type, duration_count: item.duration.type === 'months' ? item.duration.count : null, duration_end_date: item.duration.type === 'until' ? item.duration.endDate : null, reminder_enabled: item.reminderEnabled, notes: item.notes ?? null, active: item.active, expense_history: (item.history ?? []).map((revision) => ({ ...revision, categoryId: categoryIds.get(revision.categoryId) ?? '' })) })),
  recurring_incomes: (database.recurringIncomes ?? []).map((item) => ({ id: incomeIds.get(item.id), name: item.name, amount: item.amount, currency: item.currency, start_date: item.startDate, active: item.active, salary_history: item.history ?? [] })),
