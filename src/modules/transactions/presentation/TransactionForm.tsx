...
export function TransactionForm({ initial, defaultType = 'expense', onDone }: Props) {
  ...
  const isGoalContribution = Boolean(initial?.goalId);
  const goal = database.goals.find((item) => item.id === initial?.goalId);
  const goalCurrency = goal?.currency ?? initial?.currency ?? 'ARS';
  const isDollarTrade = type === 'saving' && !isGoalContribution;
  const isAsset = isDollarTrade || type === 'investment';
  const availableCategories = database.categories.filter((item) => isAsset || isGoalContribution || item.kind === type || item.kind === 'all' || item.id === initial?.categoryId);
  ...
  const submit = (event: FormEvent) => {
    ...
    if (installments && type === 'expense' && !initial) {
      ...
      const value = {
        ...
        currency: isGoalContribution ? goalCurrency : isDollarTrade ? 'USD' as const : type === 'investment' ? 'ARS' as const : currency,
        ...
        exchangeRate: isDollarTrade ? numericExchangeRate : undefined,
      };
  ...
  return <form onSubmit={submit} className="form-grid" noValidate>
    ...
    {isGoalContribution && <p className="form-note field--wide">El importe y la fecha también se actualizarán en el objetivo.</p>}
