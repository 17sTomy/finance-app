...
export function TransactionsPage() {
  ...
  const items = useMemo(() => monthData.transactions
    ...
    .sort((a, b) => ascending ? a.date.localeCompare(b.date) : b.date.localeCompare(a.date)), [monthData.transactions, filter, category, search, ascending]);
  ...
  return <>
    ...
    <Card className="transactions-card">
      <div className="toolbar"><label className="search-box"><Search size={18} /><input aria-label="Buscar por nombre" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Buscar movimiento..." /></label><select aria-label="Filtrar por categoría" value={category} onChange={(event) => setCategory(event.target.value)}><option value="all">Todas las categorías</option>{database.categories.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select><button className="button button--ghost" onClick={() => setAscending((value) => !value)}>Fecha {ascending ? '↑' : '↓'}</button></div>
      {items.length === 0 ? <EmptyState title="No encontramos movimientos" description="Probá cambiando los filtros o agregá el primero." action={<button className="button button--primary" onClick={() => setCreating(true)}>Agregar movimiento</button>} /> : <div className="transaction-list">{items.map((item) => {
        ...
        return <article className="transaction-row" key={item.id}>
          ...
          <div className="transaction-name"><strong>{item.name}{item.installmentNumber && <span className="installment-label">Cuota {item.installmentNumber}/{item.installmentCount}</span>}</strong><small>{categoryItem?.name ?? 'Sin categoría'} · {formatShortDate(item.date)}{item.investmentTicker ? ` · ${item.investmentQuantity} ${item.investmentTicker}` : ''}{item.exchangeRate ? ` · USD ${item.amount.toLocaleString('es-AR')} · TC AR$ ${item.exchangeRate.toLocaleString('es-AR')}` : ''}</small></div>
