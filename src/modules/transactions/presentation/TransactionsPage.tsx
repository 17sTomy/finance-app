...
export function TransactionsPage() {
  ...
  return <>
    ...
    <Card className="transactions-card">
      <div className="toolbar"><label className="search-box"><Search size={18} /><input aria-label="Buscar por nombre" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Buscar movimiento..." /></label><select aria-label="Filtrar por categoría" value={category} onChange={(event) => setCategory(event.target.value)}><option value="all">Todas las categorías</option>{database.categories.filter((item) => item.kind === 'expense' || item.kind === 'all').map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select><button className="button button--ghost" onClick={() => setAscending((value) => !value)}>Fecha {ascending ? '↑' : '↓'}</button></div>
    ...
    <ConfirmDialog open={!!deleting} message={deleting?.installmentPlanId ? 'Se eliminará esta ocurrencia. El resto de las cuotas seguirá intacto.' : 'Esta acción elimina el movimiento del mes seleccionado.'} onClose={() => setDeleting(null)} onConfirm={() => deleting && deleteTransaction(deleting.id)} />
  </>;
}
