...
export function DashboardPage() {
  ...
  return <>
    ...
    <div className="dashboard-main-grid">
      <Card className="category-card"><SectionHeader title="Gastos por categoría" /><div className="type-toggles">{(['expense', 'saving', 'investment'] as TransactionType[]).map((type) => <label key={type}><input type="checkbox" checked={included.includes(type)} onChange={() => setIncluded((current) => current.includes(type) ? current.filter((item) => item !== type) : [...current, type])} /><span>{type === 'expense' ? 'Gastos' : type === 'saving' ? 'Ahorro USD' : 'Inversiones'}</span></label>)}</div>{categoryData.length === 0 ? <EmptyState title="No hay consumos para mostrar" description="Cuando registres movimientos, vas a ver acá cómo se distribuyen entre tus categorías." /> : <div className="chart-and-legend"><div className="donut"><ResponsiveContainer width="100%" height={300}><PieChart><Pie data={categoryData} dataKey="value" innerRadius={82} outerRadius={116} paddingAngle={3} stroke="none">{categoryData.map((item) => <Cell key={item.id} fill={item.color} />)}</Pie><Tooltip formatter={(value) => new Intl.NumberFormat('es-AR', { style: 'currency', currency: 'ARS', maximumFractionDigits: 0 }).format(Number(value))} /></PieChart></ResponsiveContainer><div className="donut__center"><small>Total</small><MoneyValue value={categoryData.reduce((sum, item) => sum + item.value, 0)} /></div></div><div className="chart-legend">{categoryData.slice(0, 8).map((item) => <div key={item.id}><span style={{ background: item.color }} /><div><small>{item.name}</small><MoneyValue value={item.value} /></div></div>)}</div></div>}</Card>
      <div className="stacked-column">
        <Card><SectionHeader title="Límites del mes" action={<Link to="/planificacion?tab=limits" className="text-button">Gestionar</Link>} />{monthData.limits.length === 0 ? <EmptyState title="No tenés límites este mes" description="Definí cuánto querés destinar a cada categoría principal." /> : <div className="progress-list">{monthData.limits.map((limit) => { const category = database.categories.find((item) => item.id === limit.categoryId); const progress = limitProgress(limit, monthData, database.categories); const breakdown = limitCategoryBreakdown(limit, monthData, database.categories).filter((item) => item.category.parentId); const detail = breakdown.length > 0 ? breakdown.map(({ category: item, spent }) => `${item.name}: ${new Intl.NumberFormat('es-AR', { style: 'currency', currency: limit.currency, maximumFractionDigits: 0 }).format(spent)}`).join('\n') : undefined; return <div key={limit.id} className="progress-item" title={detail}><div><strong>{category?.name ?? 'Categoría'}</strong><span><MoneyValue value={progress.spent} /> / <MoneyValue value={progress.limitAmount} /> ({progress.configuredPercentage.toFixed(1)}%)</span></div><ProgressBar value={progress.percentage} color={category?.color} /><small>{Math.round(progress.percentage)}% utilizado{breakdown.length > 0 ? ' · pasá el mouse para ver subcategorías' : ''}</small></div>; })}</div>}</Card>
        <Card><SectionHeader title="Objetivos de ahorro" action={<Link to="/planificacion?tab=goals" className="text-button">Ver todos</Link>} />{database.goals.length === 0 ? <EmptyState title="No tenés objetivos creados" description="Creá una meta y vinculala con una categoría." /> : <div className="goal-list">{database.goals.slice(0, 2).map((goal) => { const saved = goalSavedAmount(goal, selectedMonth); const target = goalTargetAmount(goal, monthData); const progress = target > 0 ? saved / target * 100 : 0; return <div key={goal.id} className="goal-row"><div className="goal-icon" style={{ background: `${goal.color}22`, color: goal.color }}><CircleDollarSign size={20} /></div><div className="goal-content"><div><strong>{goal.name}</strong><span>{Math.round(progress)}%</span></div><ProgressBar value={progress} color={goal.color} /><small><MoneyValue value={saved} currency={goal.currency} /> de <MoneyValue value={target} currency={goal.currency} />{goal.targetMode === 'salaryPercentage' ? ` (${goal.salaryPercentage}%)` : ''}</small></div></div>; })}</div>}</Card>
      </div>
    </div>
    ...
    <Card className="month-movements-card"><SectionHeader title="Movimientos del mes" action={<Link to="/movimientos" className="text-button">Abrir movimientos</Link>} />{monthMovements.length === 0 ? <p className="muted compact-empty">Todavía no hay movimientos registrados en este mes.</p> : <div className="month-movements">{monthMovements.map((item) => <div className="month-movement-row" key={item.id}><span className={`transaction-icon transaction-icon--${item.type}`}>{isIncoming(item) ? <ArrowUpRight size={17} /> : <ArrowDownRight size={17} />}</span><div><strong>{item.name}</strong><small>{formatShortDate(item.date)}{item.installmentNumber ? ` · Cuota ${item.installmentNumber}/${item.installmentCount}` : ''}{item.investmentTicker ? ` · ${item.investmentQuantity} ${item.investmentTicker}` : ''}{item.exchangeRate != null ? ` · USD ${item.amount.toLocaleString('es-AR')}` : ''}</small></div><MoneyValue value={displayedMovement(item).value} currency={displayedMovement(item).currency} signed /></div>)}</div>}</Card>
