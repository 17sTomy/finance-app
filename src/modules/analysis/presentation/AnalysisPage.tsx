...
export function AnalysisPage() {
  ...
  return <>
    ...
    <div className="analysis-grid">
      <Card className="chart-card chart-card--expenses"><SectionHeader title="Gastos por categoría" />{categories.length === 0 ? <EmptyState title="No hay gastos para analizar" description="Registrá movimientos para ver su distribución por categoría." /> : <><ResponsiveContainer width="100%" height={330}><PieChart><Pie data={categories} dataKey="value" innerRadius={72} outerRadius={118} stroke="none" paddingAngle={2}>{categories.map((item) => <Cell key={item.id} fill={item.color} />)}</Pie><Tooltip formatter={(value) => formatMoney(Number(value))} /></PieChart></ResponsiveContainer><div className="compact-legend">{categories.slice(0, 6).map((item) => <span key={item.id}><i style={{ background: item.color }} />{item.name}</span>)}</div></>}</Card>
