...
import { calculateSummary, expensesByCategory } from '../../finance/domain/financeSelectors';
...
export function AnalysisPage() {
  ...
  return <>
    ...
    <div className="analysis-kpis"><Card><span className="soft-icon soft-icon--green"><ArrowUpRight size={18} /></span><small>Ingresos totales</small><MoneyValue value={summary.income} /></Card><Card><span className="soft-icon soft-icon--coral"><ArrowDownRight size={18} /></span><small>Gastos totales</small><MoneyValue value={summary.expenses} /></Card><Card><span className="soft-icon"><PiggyBank size={18} /></span><small>Ahorro USD</small><MoneyValue value={usdSummary.savings} currency="USD" /></Card><Card><span className="soft-icon soft-icon--blue"><Landmark size={18} /></span><small>Inversiones</small><MoneyValue value={summary.investments} /></Card><Card className="analysis-balance"><span className="soft-icon"><Wallet size={18} /></span><small>Balance</small><MoneyValue value={summary.balance} /></Card></div>
    <div className="analysis-grid">
      ...
      <Card className="chart-card"><SectionHeader title="Fijos vs. variables" /><ResponsiveContainer width="100%" height={280}><BarChart data={[{ name: 'Fijos', value: summary.fixedExpenses, fill: '#9b87d3' }, { name: 'Variables', value: summary.variableExpenses, fill: '#f19a8e' }]} margin={{ top: 20, right: 10, left: -10 }}><CartesianGrid strokeDasharray="3 3" stroke="#ebe7f1" vertical={false} /><XAxis dataKey="name" axisLine={false} tickLine={false} /><YAxis axisLine={false} tickLine={false} tickFormatter={(value) => `${Math.round(value / 1000)}k`} /><Tooltip formatter={(value) => formatMoney(Number(value))} /><Bar dataKey="value" radius={[10, 10, 0, 0]}>{['#9b87d3', '#f19a8e'].map((fill) => <Cell key={fill} fill={fill} />)}</Bar></BarChart></ResponsiveContainer></Card>
