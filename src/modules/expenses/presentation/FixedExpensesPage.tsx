...
function SalaryForm({ initial, onDone }: { initial?: RecurringIncome; onDone: () => void }) {
  const { saveRecurringIncome, selectedMonth } = useFinance(); const [name, setName] = useState(initial?.name ?? 'Sueldo'); const [amount, setAmount] = useState(initial?.amount ? String(initial.amount) : ''); const [currency, setCurrency] = useState<Currency>(initial?.currency ?? 'ARS'); const [startDate, setStartDate] = useState(initial?.startDate ?? `${selectedMonth}-01`); const [error, setError] = useState('');
...
export function FixedExpensesPage() {
  ...
  return <>
    ...
    <div className="subsection-heading"><div><p className="eyebrow">INGRESOS AUTOMÁTICOS</p><h2>Sueldo recurrente</h2><p className="muted small-copy">Condiciones de {selectedMonth}. Editar, pausar o reactivar se aplica desde este mes en adelante.</p></div><button className="button button--ghost" onClick={() => setSalaryEditing(null)}><Plus size={17} /> Agregar</button></div>
    {database.recurringIncomes.length === 0 ? <Card><EmptyState title="No tenés ingresos recurrentes" description="Agregá tu sueldo u otro ingreso que se repita todos los meses." action={<button className="button button--primary" onClick={() => setSalaryEditing(null)}>Crear ingreso recurrente</button>} /></Card> : <div className="fixed-grid salary-grid">{database.recurringIncomes.map((income) => recurringIncomeForMonth(income, selectedMonth)).map((item) => <Card key={item.id} className={!item.active ? 'fixed-card--paused' : ''}><div className="fixed-card__top"><span className="category-icon"><Wallet size={19} /></span><span className={`status-pill ${item.active ? 'status-pill--active' : ''}`}>{item.active ? 'Activo' : 'Pausado'}</span></div><h2>{item.name}</h2><MoneyValue value={item.amount} currency={item.currency} className="fixed-amount" /><p className="muted small-copy">Se acredita el primer día hábil argentino desde {new Intl.DateTimeFormat('es-AR').format(new Date(`${item.startDate}T12:00:00`))}; incluye feriados nacionales.</p><div className="fixed-actions"><button className="button button--ghost" onClick={() => toggleRecurringIncome(item.id)}>{item.active ? <Pause size={16} /> : <Play size={16} />}{item.active ? 'Pausar' : 'Reactivar'}</button><button className="icon-button" aria-label={`Editar ${item.name}`} onClick={() => setSalaryEditing(item)}><Pencil size={17} /></button></div></Card>)}</div>}
