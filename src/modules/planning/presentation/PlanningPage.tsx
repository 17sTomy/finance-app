...
function GoalForm({ initial, onDone }: { initial?: SavingsGoal; onDone: () => void }) {
  ...
  return <form className="form-grid" onSubmit={submit}>
    ...
    <label>{targetMode === 'amount' ? 'Monto objetivo' : 'Porcentaje del sueldo'}<input type="number" min={targetMode === 'salaryPercentage' ? '0.1' : '0.01'} max={targetMode === 'salaryPercentage' ? 100 : undefined} step={targetMode === 'salaryPercentage' ? '0.1' : '0.01'} value={value} onChange={(event) => setValue(event.target.value)} /></label>
