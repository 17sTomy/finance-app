...
const categories = [
  ...
  { id: 'sports', name: 'Deportes', icon: '🏅', color: '#6fa98f', kind: 'expense' as const },
  { id: 'gym-category', name: 'Gimnasio', icon: '🏋️', color: '#7fc8b0', kind: 'expense' as const, parentId: 'sports' },
  { id: 'basketball', name: 'Básquet', icon: '🏀', color: '#f3a66f', kind: 'expense' as const, parentId: 'sports' },
...
const fixedExpenses = [
  ...
  { id: 'gym', name: 'Gimnasio', amount: 27000, currency: 'ARS' as const, categoryId: 'gym-category', startDate: '2026-04-01', dueDay: 10, duration: { type: 'months' as const, count: 12 }, reminderEnabled: false, active: true },
...
const variableByMonth: Record<string, Transaction[]> = {
  ...
  '2026-08': [
    ...
    tx('aug-basketball', 'Cuota de básquet', 23000, '2026-08-17', 'expense', 'basketball'),
...
export function createMonth(year: number, month: number, database: Pick<FinanceDatabase, 'fixedExpenses' | 'recurringIncomes' | 'installmentPlans'>, includeDemoTransactions = false, fixedExpensesDueBy?: string): MonthlyFinanceData {
  ...
  return {
    ...
    limits: includeDemoTransactions ? [
      ...
      { id: `limit-sports-${key}`, categoryId: 'sports', percentage: 4, currency: 'ARS' },
    ] : [],
...
export function createDemoDatabase(): FinanceDatabase {
  const base: FinanceDatabase = {
    ...
    installmentPlans,
    goals: [
      { id: 'trip', name: 'Viaje', targetAmount: 1000000, currency: 'ARS', targetDate: '2027-01-15', color: '#9b87d3', categoryId: 'outings', contributions: [{ id: 'trip-1', amount: 600000, date: '2026-08-20' }] },
      { id: 'emergency', name: 'Fondo de emergencia', targetAmount: 0, targetMode: 'salaryPercentage', salaryPercentage: 15, currency: 'ARS', color: '#7fc8b0', categoryId: 'savings', contributions: [{ id: 'emergency-1', amount: 180000, date: '2026-08-20' }] },
    ],
  };
